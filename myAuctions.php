                ...
                <div class="col-6">
                    ...
                    <?php      
                        ...
                        if ($status == 'active') {
                            ...
                            // echo "<button type='button' data-toggle='modal' data-target='#rate$auctionid' class='btn btn-sm btn-outline-warning m-1'>Rate</button>";
                            require('connection.php');
                            $sql = $db->prepare("SELECT * FROM ratings WHERE auctionid=? AND sellerRating IS NOT NULL");
                            $sql->execute(array($auctionid));
                            $ratings = $sql->fetch();
                            $ratingsCount = $sql->rowCount();
                            
                            if ($ratingsCount == 0) {
                                echo '<form action="rating.php" method="POST" class="d-inline">';
                                echo '<input type="hidden" name="rater" value="seller">';
                                echo "<input type='hidden' name='auctionid' value='$auctionid'>";
                                echo '<input type="submit" name="rating" value="Rate" class="btn btn-sm btn-outline-warning m-1">';
                                echo '<input type="number" name="rate" min="0" max="5" class="m-1" placeholder="rate" required>';
                                echo '</form>';
                            } elseif ($ratingsCount > 0) {
                                $rating = $ratings['sellerRating'];
                                echo "<span class='h4 text-warning mt-3 pt-4 ml-4'>Rated $rating</span>";
                            }                        
                            // require('connection.php');
                            ...
                            //         for ($i=5; $i > 0; $i--) { 
                            //             if (isset($stars) && $i==$stars) {
                            //                 echo "<input type='radio' id='$auctionid-star$i' name='rate' class='star' value='$i' checked/>";
                            //             } else {
                            //                 echo "<input type='radio' id='$auctionid-star$i' name='rate' class='star' value='$i' $attribute/>";
                            //             }
                            //             echo "<label for='$auctionid-star$i' title='text' style=\"content: '★';\">$i stars</label>";
                            //         }
                            //     echo '</div>';
                            // echo '</form>';
            ...
            <li class="row list-group-item list-group-item-action">
                ...
                <div class="col-6">
                    ...
                    <?php
                        $auctionid = $bidding['id'];
                        echo "<a href='chat.php?auctionid=$auctionid' class='btn btn-sm btn-outline-secondary m-1'>Chat</a>";
                        // echo "<a href='rating.php?auctionid=$auctionid&rater=buyer' class='btn btn-sm btn-outline-warning m-1'>Rate</a>";
                        require('connection.php');
                        $sql = $db->prepare("SELECT * FROM ratings WHERE auctionid=? AND buyerRating IS NOT NULL");
                        $sql->execute(array($auctionid));
                        $ratings = $sql->fetch();
                        $ratingsCount = $sql->rowCount();
                        ...
                        if ($ratingsCount == 0) {
                            echo '<form action="rating.php" method="POST" class="d-inline">';
                            echo '<input type="hidden" name="rater" value="buyer">';
                            echo "<input type='hidden' name='auctionid' value='$auctionid'>";
                            echo '<input type="submit" name="rating" value="Rate" class="btn btn-sm btn-outline-warning m-1">';
                            echo '<input type="number" name="rate" min="0" max="5" class="m-1" placeholder="rate" required>';
                            echo '</form>';
                        } elseif ($ratingsCount > 0) {
                            $rating = $ratings['buyerRating'];
                            echo "<span class='h4 text-warning mt-3 pt-4 ml-4'>Rated $rating</span>";
                        }
                    }
