...
$username = $user;
$sql = $db->prepare("SELECT * FROM auctions, ratings WHERE ratings.auctionid = auctions.id AND auctions.owner=?");
...
$sql = $db->prepare("SELECT * FROM auctions, ratings WHERE ratings.auctionid = auctions.id AND auctions.bidder=?");
$sql->execute(array($username));
$holder = $sql->fetchAll();
            ...
            <li class="row list-group-item list-group-item-action">
                ...
                <div class="col-6">
                    ...
                    <?php      
                        ...
                        if ($status=='active' && isset($_SESSION['username']) && $_SESSION['username']!=$auction['owner']) {
